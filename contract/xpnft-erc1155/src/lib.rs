use near_contract_standards::multi_token::metadata::{MT_METADATA_SPEC, MultiTokenMetadataProvider};
use near_contract_standards::multi_token::token::{Token, TokenId};
use near_contract_standards::multi_token::events::MtMint;
...
impl Contract {
    ...
    pub fn mt_mint(
        ...
        let token = self.tokens
            .internal_mint(token_owner_id, Some(supply), Some(token_metadata), None);
